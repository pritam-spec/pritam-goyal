...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson")
SENT_IMG_DIR = Path("output_reduce/images")
SENT_PDF_DIR = Path("output_reduce/pdfs")
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # call LLM worker (this should already pass tables_only flag)
    ...
    # --- TABLES-ONLY mode: strictly no narrative, no fallback ---
    if SEND_TABLES_ONLY:
        # Ensure tables/charts exist as lists
        tables = page_obj.get("tables") or []
        charts = page_obj.get("charts") or []

        # Forcefully drop any narrative/title/notes that might have been returned
        page_obj["narrative_text"] = None
        page_obj["title"] = None
        page_obj["subtitle"] = None
        page_obj["notes"] = None

        # Normalize fields
        page_obj["tables"] = tables
        page_obj["charts"] = charts
        page_obj.setdefault("debug", {})["tables_only_mode"] = True

        # If nothing structured was detected, emit explicit no-detection
        if (not tables) and (not charts):
            page_obj.update({
                "error": "no_tables_or_charts",
                "debug": {**page_obj.get("debug", {}), "skipped_local_text_extraction": True},
            })
        else:
            page_obj.setdefault("debug", {})["kept_structured_only"] = True

        # Save and return (do not run _fallback_scrape_page under any circumstance)
    else:
        # --- Non TABLES-ONLY: original fallback behavior preserved ---
        need_fallback = not (isinstance(page_obj, dict) and (page_obj.get("narrative_text") or "").strip())
        if need_fallback:
            try:
                fb = _fallback_scrape_page(pdf_path, pno)
                if fb:
                    # Use existing merge helper to combine LLM + fallback text (safe)
                    page_obj = _merge_page_objs([page_obj, fb])
                    page_obj.setdefault("debug", {})["local_fallback_used"] = True
            except Exception as _e:
                logger.warning("Local fallback failed for page %d: %s", pno, _e)
