...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_1")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_1")
SENT_IMG_DIR = Path("output_reduce_1/images")
SENT_PDF_DIR = Path("output_reduce_1/pdfs")
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Merge local structured text into page_obj, without allowing LLM narrative to be kept
    # Keep LLM tables/charts as authoritative for structured visuals.
    try:
        # Always strip any narrative text returned by LLM (we only want local text for narrative)
        page_obj["narrative_text"] = None

        # Prefer title/subtitle from LLM only if local doesn't provide; otherwise prefer local
        ...
        # Attach structured sections (local only) under a new key "sections"
        # This preserves the hierarchy user requested (heading -> content).
        page_obj["sections"] = local_struct.get("sections") or []

        # Optionally keep raw lines for debugging; controlled via debug flag
        if local_struct.get("raw_lines"):
            page_obj.setdefault("debug", {})["local_raw_lines_count"] = len(local_struct.get("raw_lines") or [])
