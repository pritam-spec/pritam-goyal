...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_4")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_4")
SENT_IMG_DIR = Path("output_reduce_4/images")
SENT_PDF_DIR = Path("output_reduce_4/pdfs")
...
def _render_crop_png_bytes(pdf_path: Path, page_no: int, rect_tuple):
    """
    Render a crop of the page to PNG bytes. rect_tuple is (x0,y0,x1,y1) in page coords.
    """
    try:
        import fitz
        ...
        # scale matrix based on device pixel ratio — use 2.0 for better recognition
...
def _table_is_duplicate(existing_tables: list, new_table: dict) -> bool:
    """
    Determine if new_table is duplicate of any existing table (conservative).
    Compares normalized headers and first row overlap.
    """
    def norm_list(lst):
    ...
    for et in (existing_tables or []):
        ...
        # header exact match or large overlap
        if new_headers and eh and (set(new_headers) == set(eh) or (len(set(new_headers).intersection(eh)) / max(1, len(new_headers)) ) >= 0.7):
            return True
        # first-row overlap
        if new_first and ef and (len(set(new_first).intersection(ef)) / max(1, len(new_first))) >= 0.6:
            return True
    return False
...
def _repair_llm_tables_with_local(page_obj: dict, local_struct: dict) -> dict:
    """
    Try to repair LLM-detected tables using local spans & x positions.
    For each LLM table:
      - find a header raw_line (matching header tokens) and capture header x positions
      - for each row, find the raw_line(s) that start with the row label and then collect numeric spans,
        assigning numeric spans to nearest header by x.
    This is best-effort and won't break existing tables; returns modified page_obj.
    """
    try:
        raw_lines = local_struct.get("raw_lines") or []
        # prepare a searchable map of normalized raw span tokens -> span entries (x,y)
        span_index = []
        for rl in raw_lines:
        ...
        for tbl in (page_obj.get("tables") or []):
            ...
            if not headers:
                continue
            # build header x positions using first raw_line that contains all headers (best-effort)
            header_positions = []
            # try to find a line containing a header token (normalized)
            for rl in raw_lines:
                nline = _norm_token(rl["text"])
                # check how many headers tokens are inside this line
                hits = 0
                for h in headers:
                    if h and _norm_token(str(h)) in nline:
                        hits += 1
                if hits >= max(1, len(headers)//3):  # partial match acceptable
                    # estimate column positions by looking at spans on the same rl (their x)
                    spans = rl.get("raw_spans", [])
                    # map normalized span text -> x
                    hp = []
                    for h in headers:
                        nh = _norm_token(str(h))
                        # find span with that token or closest token
                        candidate = None
                        for s in spans:
                            if _norm_token(s["text"]) == nh:
                                candidate = s
                                break
                        if candidate:
                            hp.append(candidate["x"])
                        else:
                            # fallback: try approximate by scanning span_index for token on same y
                            found = next((si for si in span_index if si["norm"] == nh and abs(si["y"] - rl["y"]) < 3.0), None)
                            hp.append(found["x"] if found else None)
                    header_positions = hp
                    break

            # if header_positions empty: skip repair for this table
            if not header_positions or all(h is None for h in header_positions):
                continue

            # For each table row, try to find the corresponding raw_line by matching the row label text
            repaired_rows = []
            for row in (tbl.get("rows") or []):
                if not row:
                ...
                if not row_label:
                    repaired_rows.append(row)
                    continue
                # find raw line whose text contains normalized row_label
                candidate_line = None
                for rl in raw_lines:
                    if _norm_token(row_label) in _norm_token(rl["text"]):
                        candidate_line = rl
                        break
                ...
                # collect numeric spans on candidate_line and its immediate neighbors (same y band)
                spans = candidate_line.get("raw_spans", [])[:]
                # also look at nearby lines with similar y (some tables wrap)
                for rl2 in raw_lines:
                    if rl2 is candidate_line:
                        continue
                    if abs(rl2["y"] - candidate_line["y"]) < 6.0:
                        spans.extend(rl2.get("raw_spans", []))

                # keep only numeric-like tokens or tokens that look like table cells
                cell_spans = []
                for s in spans:
                    t = s.get("text") or ""
                    tn = t.replace(",", "")
                ...
                # assign each cell_span to nearest header column by x distance
                assigned = ["" for _ in headers[1:]]  # headers[0] is row label
                for sx, tv in cell_spans:
                    # compute distances to header_positions
                    best_i, best_d = None, None
                    for i, hx in enumerate(header_positions[1:], start=0):  # header_positions include first empty header often
                        if hx is None:
                            continue
                        d = abs(sx - hx)
                    ...
                    if best_i is not None:
                        # place tv in assigned[best_i]; if already present, append (keep order)
                        if assigned[best_i]:
                            assigned[best_i] = assigned[best_i] + " " + tv
                        else:
                            assigned[best_i] = tv

                # build repaired row: [label] + assigned columns; ensure length matches headers
                new_row = [row_label] + [c.strip() if c else "" for c in assigned]
            ...
            # replace rows only if repaired rows look meaningfully numeric (we do light check)
            # compute fraction of numeric tokens in repaired rows vs original
            def numeric_fraction(rows):
                num, tot = 0, 0
                for r in rows:
                    for cell in r[1:]:
                        if cell and _RE_NUMERIC.match(str(cell).replace(",", "")):
                            num += 1
                        tot += 1
                return (num / tot) if tot > 0 else 0.0

            orig_frac = numeric_fraction(tbl.get("rows") or [])
            new_frac = numeric_fraction(repaired_rows)
            # Accept repair if new_frac >= orig_frac (we didn't lose numeric structure)
            if new_frac >= orig_frac and new_frac > 0:
                tbl["rows"] = repaired_rows
    except Exception as e:
        logger.debug("Table repair failed on page: %s", e)
    return page_obj
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 2) LOCAL: extract textual structure
    local_struct = {}
    try:
    ...
    # Filter local sections that duplicate LLM results
    try:
    ...
    # ------------------ NEW: detect table-like areas and send crops to LLM ------------------
    try:
        bboxes = _detect_table_bboxes_from_local(filtered_local or local_struct)
        if bboxes:
            # for each bbox, render crop and call the LLM on that crop image
            for rect in bboxes:
                crop_bytes = _render_crop_png_bytes(pdf_path, pno, rect)
                if not crop_bytes:
                    continue
                try:
                    # call LLM on the crop explicitly for tables only
                    crop_result = extract_info_from_image(crop_bytes, pno, fitz_page=None, mime_type="image/png", tables_only=True)
                    # If crop_result contains tables, merge de-duplicatively
                    for nt in (crop_result.get("tables") or []):
                        if not _table_is_duplicate(page_obj.get("tables", []), nt):
                            page_obj.setdefault("tables", []).append(nt)
                    # Also merge charts from crop_result if present and not duplicates (simple approach)
                    for nc in (crop_result.get("charts") or []):
                        # naive dup check: compare title or data length
                    ...
                    logger.debug("LLM crop extraction failed for page %d rect %s: %s", pno, rect, e)
    except Exception as e:
        logger.debug("Table bbox detection/processing failed for %s page %d: %s", pdf_path, pno, e)

    # ------------------ NEW: run table repair using the (possibly updated) local raw lines ------------------
    try:
        page_obj = _repair_llm_tables_with_local(page_obj, filtered_local or local_struct)
    except Exception as e:
        logger.warning("Table repair failed for %s page %d: %s", pdf_path, pno, e)

    # ------------------ NEW: local chart detection fallback ------------------
    try:
        page_obj = _detect_charts_from_local_text(page_obj, filtered_local or local_struct)
    except Exception as e:
        logger.debug("Local chart detection failed for %s page %d: %s", pdf_path, pno, e)
