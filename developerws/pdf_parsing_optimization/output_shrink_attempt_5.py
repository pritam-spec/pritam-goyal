...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_5")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_5")
SENT_IMG_DIR = Path("output_reduce_5/images")
SENT_PDF_DIR = Path("output_reduce_5/pdfs")
...
# ---------------------------------------------------------
# Prompt for running LLM *only* on charts (minimizes tokens)
# ---------------------------------------------------------
CHART_ONLY_PROMPT = r"""
ONLY EXTRACT CHARTS — DO NOT RETURN ANY TABLES, NARRATIVE, TITLES,
FOOTERS, PAGE INFO, OR ANY EXTRA TEXT.  Return a strict JSON array
(list) of chart objects following exactly this schema and nothing else:
...
[
  {
    ...
    "x_axis": {"label": "<x label or null>", "values": ["v1","v2", ...]},
    "y_axis": {"label": "<y label or null>", "values": [num1, num2, ...]},
    "data_series": [{"name":"<series name or null>", "values":[num,...]}, ...],
...
- If a chart has no numeric series, put an empty array.
- Do not include any narrative, tables, or debugging text.
- If no charts are present on the page, return an empty JSON array: [].
"""
...
def extract_tables_locally(pdf_path: Path, page_no: int) -> List[Dict[str, Any]]:
    """Simple local table extractor using PyMuPDF text blocks (fast heuristic)."""
    ...
    try:
        ...
        with fitz.open(str(pdf_path)) as doc:
            ...
            for b in d.get("blocks", []):
                # Heuristic: blocks with many newline-separated lines and roughly column-like content
                if b.get("type") == 0:  # text block
                    ...
                    for l in b.get("lines", []):
                        for s in l.get("spans", []):
                            text += s.get("text", "")
                        text += "\n"
                    # look for patterns that resemble table (multiple rows and columns)
                    lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
                    if len(lines) >= 2:
                        # split by 2+ spaces or tab to guess columns
                        rows = []
                        for ln in lines:
                            ...
                            if len(cols) > 1:
                                rows.append(cols)
                        if rows:
                            # create header if first row seems header-like
                            headers = rows[0] if len(rows) > 0 else [f"col_{i+1}" for i in range(len(rows[0]))]
                            out_tables.append({"type": "table", "title": None, "headers": headers, "rows": rows[1:] if len(rows)>1 else []})
    except Exception:
        # swallow; calling code will handle empty list
        pass
    return out_tables
...
def extract_info_from_image(
    png_bytes_unused: bytes, page_num: int, fitz_page=None, mime_type: str = "image/png", charts_only: bool = False
) -> Dict[str, Any]:
    ...
    # choose prompt depending on charts_only flag
    prompt_to_send = CHART_ONLY_PROMPT if charts_only else BASE_PROMPT
    ...
    for attempt in range(1, MAX_MODEL_RETRIES + 1):
        try:
            ...
            logger.info("LLM call page=%d took %.2fs (charts_only=%s)", page_num, time.time() - t0, charts_only)

            inp_tokens = usage.get("input_tokens")
            out_tokens = usage.get("output_tokens")
            if inp_tokens is None:
                inp_tokens = _approx_tokens_from_text(prompt_to_send) + int(est_image_tokens or 0)
            if out_tokens is None:
                out_tokens = _approx_tokens_from_text(raw_text)
            TOK.add(inp_tokens, out_tokens)

            # strip code fences if any
            raw_text = re.sub(r"```(?:json)?|```", "", raw_text or "").strip()

            # ---------- ATTEMPT TO PARSE JSON ----------
            # prefer extracting explicit JSON candidates if the helper exists
            ...
            # If no candidate parsed, try direct parse of the whole raw_text
            if parsed_obj is None:
                parsed_obj = _try_load_json(raw_text)
            ...
            # Normalise parsed_obj into expected page dict shape.
            # Cases:
            #  - parsed_obj is a dict => assume full page object (may contain charts/tables/narrative)
            #  - parsed_obj is a list of chart dicts => put under "charts"
            #  - parsed_obj is a list of other => wrap as {"value": parsed_obj}
            data = None
            if isinstance(parsed_obj, dict):
                data = parsed_obj
            elif isinstance(parsed_obj, list):
                # if list of dicts that look like charts, put under "charts"
                is_chart_list = (
                    len(parsed_obj) > 0
                    and all(isinstance(x, dict) for x in parsed_obj)
                    and any(("type" in x and (str(x.get("type")).lower() == "chart" or "chart" in str(x.get("type")).lower()) )
                            or ("chart_type" in x) for x in parsed_obj)
                )
                if is_chart_list:
                    data = {"charts": parsed_obj}
                else:
                    # maybe single-element list with dict inside (common in some outputs)
                    if len(parsed_obj) == 1 and isinstance(parsed_obj[0], dict):
                        data = parsed_obj[0]
                    else:
                        data = {"value": parsed_obj}
            else:
                data = {"value": parsed_obj}

            # ensure some keys exist
            data.setdefault("tables", data.get("tables") or [])
            data.setdefault("charts", data.get("charts") or [])
            ...
            data["debug"].update({
                ...
                "image_px": [int(sent_w) if sent_w else 0, int(sent_h) if sent_h else 0],
                "image_token_est": int(est_image_tokens or 0),
                "prompt_used": "CHART_ONLY_PROMPT" if charts_only else "BASE_PROMPT",
            })
            return data
            ...
            logger.error(
                ...
                _llm_provider_label(), page_num, attempt, MAX_MODEL_RETRIES, e,
            )
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1) Call LLM only for charts (minimizes output tokens)
    page_obj = extract_info_from_image(img_bytes, pno, fitz_page=None, mime_type=mime, charts_only=True)

    # 2) Always run local narrative extractor (PyMuPDF fallback)
    try:
        local_narrative = _fallback_scrape_page(pdf_path, pno) or {}
    except Exception as e:
        logger.warning("Local narrative extraction failed for %s page %d: %s", pdf_path.name, pno, e)
        local_narrative = {}

    # 3) Always run local table extraction (fast heuristic or camelot/tabula if available)
    try:
        local_tables = extract_tables_locally(pdf_path, pno) or []
    except Exception as ex:
        logger.warning("local table extraction failed for %s page %d: %s", pdf_path.name, pno, ex)
        local_tables = []

    # 4) Merge charts from LLM with local narrative and local tables.
    # Ensure page_obj is a dict
    if not isinstance(page_obj, dict):
        page_obj = {"page_number": pno, "charts": [], "tables": [], "narrative_text": None, "debug": {}}

    # incorporate local narrative if missing or empty
    # prefer LLM narrative only if it exists (unlikely since charts_only=True)
    if not (page_obj.get("narrative_text") and str(page_obj.get("narrative_text")).strip()):
        page_obj = _merge_page_objs([page_obj, local_narrative])

    # merge local tables (append or set if none)
    existing_tables = page_obj.get("tables") or []
    # avoid duplicating if LLM already returned tables (rare when charts_only)
    if local_tables:
        page_obj["tables"] = existing_tables + local_tables

    # ensure page_number is set
    page_obj["page_number"] = pno

    # write raw page JSON (same as before)
