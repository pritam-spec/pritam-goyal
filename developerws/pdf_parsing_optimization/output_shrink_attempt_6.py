...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_6")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_6")
SENT_IMG_DIR = Path("output_reduce_6/images")
SENT_PDF_DIR = Path("output_reduce_6/pdfs")
...
# --- Crop / repair tuning (env overrides) ---
MAX_CROP_LLM_CALLS = int(os.getenv("MAX_CROP_LLM_CALLS", "1"))         # max LLM crop calls per page (keep small)
CROP_RENDER_SCALE = float(os.getenv("CROP_RENDER_SCALE", "1.2"))       # scale for crop rendering (lower -> smaller image)
REPAIR_ROW_COUNT_TOLERANCE = float(os.getenv("REPAIR_ROW_COUNT_TOLERANCE", "0.2"))  # allowed relative row count delta
REPAIR_MIN_IMPROVEMENT = float(os.getenv("REPAIR_MIN_IMPROVEMENT", "0.05"))        # accept repair only if numeric fraction improves by this margin
...
def _render_crop_png_bytes(pdf_path: Path, page_no: int, rect_tuple):
    ...
    Uses CROP_RENDER_SCALE to control output size (lower -> smaller image -> fewer tokens).
    """
    try:
        import fitz
        ...
        # scale matrix based on config constant
...
def _repair_llm_tables_with_local(page_obj: dict, local_struct: dict) -> dict:
    """
    Conservative table repair: use local spans to align numeric cells to headers, but:
      - DO NOT insert rows that are category headers (no numeric cells)
      - Accept repair only if numeric_fraction improves by REPAIR_MIN_IMPROVEMENT
        and the repaired row count isn't wildly different (within REPAIR_ROW_COUNT_TOLERANCE)
    """
    try:
        raw_lines = local_struct.get("raw_lines") or []
        span_index = []
        ...
        for tbl in (page_obj.get("tables") or []):
            ...
            for row in orig_rows:
                ...
                # find raw line matching row label
                candidate_line = next((rl for rl in raw_lines if _norm_token(row_label) in _norm_token(rl["text"])), None)
                if not candidate_line:
                    # no mapping found -> keep original
                    repaired_rows.append(row)
                    continue

                # collect numeric-like spans near candidate_line
                ...
                # if no numeric spans found near this label -> this is likely a category header; keep original row
                if not cell_spans:
                    repaired_rows.append(row)
                    continue

                # assign numeric spans to nearest header (skip headers[0], it's the row label)
            ...
            # evaluate numeric fraction improvement
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ------------------ NEW: detect table-like areas and send up to MAX_CROP_LLM_CALLS crops to LLM ------------------
    try:
        # Only attempt crop-LM if we have few or no tables from initial LLM pass.
        # This avoids extra LLM calls on pages already well-handled by the LLM.
        if len(page_obj.get("tables") or []) == 0:
    ...
    # ------------------ run conservative table repair and chart detection ------------------
    try:
