...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_7")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_7")
SENT_IMG_DIR = Path("output_reduce_7/images")
SENT_PDF_DIR = Path("output_reduce_7/pdfs")
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1) initial LLM pass (tables & charts only)
    ...
    # 2) local structured text extraction
    try:
        local_struct = _local_extract_text_structured(pdf_path, pno) or {}
    except Exception as e:
        logger.warning("Local extraction failed for %s page %d: %s", pdf_path, pno, e); local_struct = {}

    # 3) filter local text duplicates
    try:
        filtered_local = _filter_local_struct_by_llm(page_obj, local_struct)
    except Exception as e:
        logger.warning("Filter failed for %s page %d: %s", pdf_path, pno, e); filtered_local = local_struct or {}
    ...
    # 4) FIND priority candidates: heading keywords (SIP etc) + glossary + regular table bands
    try:
        candidates = []
        # priority headings first (for SIP etc)
        heading_rects = _find_priority_heading_bboxes(filtered_local or local_struct)
        for r in heading_rects:
            candidates.append({"rect": r, "type": "keyword", "score": 100})
        # then glossary/regular table candidates
        for c in _find_table_candidates(filtered_local or local_struct):
            # only add if not overlapping an identical candidate already added
            candidates.append(c)
        # sort by score desc (keyword first etc)
        candidates = sorted(candidates, key=lambda x: x.get("score", 0), reverse=True)
    except Exception as e:
        logger.debug("Candidate detection failed for %s page %d: %s", pdf_path, pno, e); candidates = []

    # 5) Conservative crop-LM calls: limit by MAX_CROP_LLM_CALLS
    try:
        crop_calls = 0
        # Only attempt crops if page has no tables OR candidate is keyword/glossary (priority)
        for cand in candidates:
            if crop_calls >= MAX_CROP_LLM_CALLS:
                break
            cand_type = cand.get("type")
            # only run crops when either no tables found OR candidate is high priority
            if page_obj.get("tables") and len(page_obj.get("tables")) > 0 and cand_type == "regular":
                # skip regular candidates if LLM already returned some tables
                continue
            rect = cand.get("rect") if isinstance(cand.get("rect"), tuple) else cand.get("rect")
            if not rect:
                continue
            crop_bytes = _render_crop_png_bytes(pdf_path, pno, rect)
            if not crop_bytes:
                continue
            try:
                crop_result = extract_info_from_image(crop_bytes, pno, fitz_page=None, mime_type="image/png", tables_only=True)
                # merge crop tables only if non-duplicate
                for nt in (crop_result.get("tables") or []):
                    if not _table_is_duplicate(page_obj.get("tables", []), nt):
                        page_obj.setdefault("tables", []).append(nt)
                for nc in (crop_result.get("charts") or []):
                    if not any((nc.get("title") and nc.get("title")==ec.get("title")) for ec in page_obj.get("charts") or []):
                        page_obj.setdefault("charts", []).append(nc)
            except Exception as e:
                logger.debug("Crop LLM failed for %s page %d rect %s: %s", pdf_path, pno, rect, e)
            crop_calls += 1
    except Exception as e:
        logger.debug("Crop processing error for %s page %d: %s", pdf_path, pno, e)

    # 6) conservative repair and chart detection
    try:
