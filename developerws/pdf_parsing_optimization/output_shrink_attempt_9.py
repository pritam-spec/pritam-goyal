...
# I/O
PDF_INPUT_DIR = os.getenv("PDF_INPUT_DIR", "test_hdfc")
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_9")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_9")
SENT_IMG_DIR = Path("output_reduce_9/images")
SENT_PDF_DIR = Path("output_reduce_9/pdfs")
...
def _detect_glossary_candidate(local_struct: dict):
    """
    Detect a glossary-like two-column region: many sequential lines where each line has exactly 2 spans.
    Return a single rect (x0,y0,x1,y1) or None.
    """
    raw_lines = local_struct.get("raw_lines") or []
    if not raw_lines:
        return None
    # find runs of lines where raw_spans length == 2
    run = []
    runs = []
    for ln in raw_lines:
        spans = ln.get("raw_spans") or []
        if len(spans) == 2 and ln.get("text") and len(spans[0].get("text","").strip())>0:
            run.append(ln)
        else:
            if len(run) >= 6:  # threshold: >=6 lines → likely glossary
    ...
    ys = [int(round(l["y"])) for l in best]
    x_vals = []
    for l in best:
        for s in (l.get("raw_spans") or []):
            x_vals.append(int(round(s.get("x", 0))))
    if not x_vals:
        return None
    x0 = max(0, min(x_vals) - 8)
    x1 = max(x_vals) + 50
    y0 = min(ys) - 6
    y1 = max(ys) + 12
    return (x0, y0, x1, y1)
...
def _local_extract_text_structured(pdf_path: Path, page_no: int) -> Dict[str, Any]:
    """
    Extract a richer representation of the page text layer, suitable for:
      - building sections (heading -> content)
      - repairing LLM tables by using span x positions
    Returns:
      {
        "title": <str|null>,
        "subtitle": <str|null>,
        "sections": [...],
        "raw_lines": [ { "text":..., "size":..., "bold":..., "x":..., "y":..., "raw_spans":[{"text","x","y","size","bold"}] }, ... ]
      }
    """
    ...
    for b in blocks:
        for ln in b.get("lines", []):
            spans = ln.get("spans", []) or []
            # combine spans text but keep individual spans
            span_entries = []
            line_text_parts = []
            # Use bbox from spans for coordinates
            for s in spans:
                s_text = (s.get("text") or "").strip()
                if not s_text:
                    continue
                s_size = float(s.get("size") or 0.0)
                s_font = s.get("font") or ""
                s_flags = int(s.get("flags") or 0)
                s_bbox = s.get("bbox") or []
                sx = float(s_bbox[0]) if s_bbox else 0.0
                sy = float(s_bbox[1]) if s_bbox else 0.0
                sbold = ("Bold" in s_font) or (bool(s_flags & 2))
                span_entries.append({"text": s_text, "x": sx, "y": sy, "size": s_size, "bold": bool(sbold)})
            ...
            # filter page number / trivial bullets (keep conservative)
            if _RE_PAGE.match(line_text):
                continue

            # compute aggregate properties
            size = max(se["size"] for se in span_entries) if span_entries else 0.0
            bold = any(se["bold"] for se in span_entries)
            x = span_entries[0]["x"]
            y = span_entries[0]["y"]
    ...
    # Sort by vertical then x
    raw_lines.sort(key=lambda r: (round(r["y"], 2), round(r["x"], 1)))

    # Merge very close y-lines into same paragraph cluster (prevents losing very short lines)
    merged = []
    cur = {"y": raw_lines[0]["y"], "texts": [raw_lines[0]["text"]], "raw_spans": raw_lines[0]["raw_spans"], "x": raw_lines[0]["x"], "size": raw_lines[0]["size"], "bold": raw_lines[0]["bold"]}
    for ln in raw_lines[1:]:
        if abs(ln["y"] - cur["y"]) <= LOCAL_MIN_PARAGRAPH_Y_GAP:
            # join into current cluster preserving order; also append spans
            cur["texts"].append(ln["text"])
            cur["raw_spans"].extend(ln["raw_spans"])
            # update x/size/bold heuristics if needed
    ...
    # Build lines with aggregated text
    final_lines = []
    for m in merged:
        txt = "\n".join(m["texts"]).strip()
        final_lines.append({
    ...
    out["raw_lines"] = final_lines

    # Heuristics for title/subtitle: first 3 lines
    first_lines = final_lines[: min(4, len(final_lines))]
    if first_lines:
        # choose top-most by y and largest size
        candidate = sorted(first_lines, key=lambda r: (-r["size"], r["y"]))[0]
        out["title"] = candidate["text"] if candidate["text"] and len(candidate["text"]) < 200 else None
        # subtitle candidate: next line smaller
        if len(first_lines) > 1:
            second = sorted(first_lines, key=lambda r: r["y"])[1]
            if second["size"] < candidate["size"]:
                out["subtitle"] = second["text"]

    # Build sections: treat bold/large lines as headings, otherwise content.
    sections: List[Dict[str, Any]] = []
    cur = {"heading": None, "content_lines": []}
    # Determine heading size threshold: use second-largest distinct size if possible
    sizes = sorted({round(l["size"], 2) for l in final_lines if l.get("size") and l.get("size") > 0}, reverse=True)
    ...
    for ln in final_lines:
        ...
        if heading_size and ln["size"] >= (heading_size - 0.01) and ln["text"] != out.get("title"):
            is_heading = True
        if ln["bold"] and len(ln["text"].split()) <= 7:
            is_heading = True
    ...
    # Post-process clean-up: trim and compress whitespace
    cleaned = []
    for s in sections:
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    crop_calls = 0
    try:
        # skip crop if initial LLM already provided a table with matching header/title
        def has_similar_table_for_rect_candidate():
            # simple heuristic: if page already has >=1 table, skip unless priority header exists
            return len(page_obj.get("tables") or []) > 0
        ...
        if crop_calls < MAX_PRIORITY_CROP_CALLS:
            ...
            if gl_rect:
                # If there is already a table on the page that likely corresponds to glossary title, skip.
                already = False
                for t in page_obj.get("tables") or []:
                    # if title or headers overlap with "glossary" content, skip (conservative)
                    if t.get("title") and "glossary" in (t.get("title") or "").lower():
                        already = True; break
                if not already:
        ...
        if crop_calls < MAX_PRIORITY_CROP_CALLS:
            ...
            if pr_rect:
                ...
                found_similar = False
                # build small set of normalized table headers/titles to compare
                existing_norms = set()
                for t in page_obj.get("tables") or []:
                ...
                # quick check: see if any priority keywords already covered
    ...
    # 6) lightweight chart detection fallback (unchanged / optional)
    try:
