...
# TUNABLES (put near other config vars)
MAX_CROP_LLM_CALLS = int(os.getenv("MAX_CROP_LLM_CALLS", "2"))   # max crops LLM per page (keep small)
CROP_RENDER_SCALE = float(os.getenv("CROP_RENDER_SCALE", "1.2")) # scale for crop rendering (smaller -> less tokens)
REPAIR_MIN_IMPROVEMENT = float(os.getenv("REPAIR_MIN_IMPROVEMENT", "0.05"))
REPAIR_ROW_COUNT_TOLERANCE = float(os.getenv("REPAIR_ROW_COUNT_TOLERANCE", "0.2"))
PRIORITY_KEYWORD_REGEXES = [
    re.compile(r"\bSIP\b", re.I),
    re.compile(r"If you had invested", re.I),
    re.compile(r"SIP - If", re.I),
    re.compile(r"Systematic Investment Plan", re.I),
]


# --- table/text repair / filter thresholds (tweak if needed) ---
TABLE_TEXT_OVERLAP_THRESHOLD = float(os.getenv("TABLE_TEXT_OVERLAP_THRESHOLD", "0.6"))  # overlap fraction to drop
TABLE_NUMERIC_MATCH_FRACTION = float(os.getenv("TABLE_NUMERIC_MATCH_FRACTION", "0.6"))
LOCAL_MIN_PARAGRAPH_Y_GAP = float(os.getenv("LOCAL_MIN_PARAGRAPH_Y_GAP", "6.0"))  # pts: gap used to join small lines into paragraphs
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output_reduce_final")
RAW_JSON_DIR = os.getenv("RAW_JSON_DIR", "output_reduce_rawjson_final")
SENT_IMG_DIR = Path("output_reduce_final/images")
SENT_PDF_DIR = Path("output_reduce_final/pdfs")
...
def _detect_table_bboxes_from_local(local_struct: dict):
    """
    Heuristic: find rectangular regions that look like multi-column rows made of spans.
    Returns list of rect tuples (x0, y0, x1, y1) in page coordinate space.
    """
    rects = []
    raw_lines = local_struct.get("raw_lines") or []
    if not raw_lines:
        return rects
    ...
    for ln in raw_lines:
        y = round(ln.get("y", 0))
        rows.setdefault(y, []).append(ln)

    # For each row, collect span x positions
    row_x_counts = []
    for y, items in rows.items():
        xs = []
        for it in items:
            for s in (it.get("raw_spans") or []):
                xs.append(float(s.get("x", 0)))
        xs = sorted(xs)
        if xs:
            row_x_counts.append((y, xs))

    if not row_x_counts:
        return rects

    # Examine vertical sequences with at least 3 rows having >=2 span clusters
    candidates = []
    for i, (y, xs) in enumerate(row_x_counts):
        if len(xs) >= 2:  # at least two columns present in the row
            candidates.append(y)

    # If not many candidate rows, nothing to do
    if len(candidates) < 3:
        return rects

    # Determine y-range of candidate rows (continuous bands)
    cand_rows = sorted(candidates)
    # group continuous sequences (gap threshold)
    groups = []
    cur = [cand_rows[0]]
    for y in cand_rows[1:]:
        if y - cur[-1] <= 20:  # 20 pts gap allowed between rows
            cur.append(y)
        else:
            groups.append(cur)
            cur = [y]
    ...
    for g in groups:
        ...
        y0 = min(g) - 6
        y1 = max(g) + 18
        # For x extents look at all spans on rows near group
        xs_all = []
        for yy in g:
            for it in rows.get(yy, []):
                for s in (it.get("raw_spans") or []):
                    xs_all.append(float(s.get("x", 0)))
        if not xs_all:
            continue
        x0 = min(xs_all) - 8
        x1 = max(xs_all) + 60  # add right padding to include cell text
        rects.append((max(x0, 0), max(y0, 0), x1, y1))

    # Optionally merge overlapping rects
    merged = []
    for r in rects:
        if not merged:
            merged.append(r)
            continue
        last = merged[-1]
        # if overlapping in y, merge
        if r[1] <= last[3] + 10 and abs(r[0] - last[2]) < 100:
            merged[-1] = (min(last[0], r[0]), min(last[1], r[1]), max(last[2], r[2]), max(last[3], r[3]))
        else:
            merged.append(r)
    return merged


# Helpers: improved candidate detection, keyword bbox finder, crop renderer, duplicate check
def _find_table_candidates(local_struct: dict):
    """
    Return candidate regions [{'rect':(x0,y0,x1,y1),'type':'regular'|'glossary','score':..}, ...]
    Conservative: require a vertical band of rows with multiple span x positions.
    """
    rects = []
    raw_lines = local_struct.get("raw_lines") or []
    if not raw_lines:
    ...
    # bucket lines by rounded y to form rows
    rows = {}
    for ln in raw_lines:
        y = int(round(ln.get("y", 0)))
        rows.setdefault(y, []).append(ln)

    # compute per-row column cluster count (unique x positions of spans)
    row_infos = []
    for y, items in rows.items():
        xs = []
        for it in items:
            for s in (it.get("raw_spans") or []):
                xs.append(int(round(s.get("x", 0))))
        xs_sorted = sorted(set(xs))
        row_infos.append((y, len(xs_sorted), xs_sorted))
    if not row_infos:
        return rects

    # candidate rows where there are >=2 span clusters (columns)
    cand_rows = [r for r in row_infos if r[1] >= 2]
    if len(cand_rows) < 2:
        # check for glossary-like: many rows with exactly 2 spans (term + definition)
        glossary_rows = [r for r in row_infos if r[1] == 2]
        if len(glossary_rows) >= 6:
            # bounding box of those rows
            ys = [r[0] for r in glossary_rows]
            y0, y1 = min(ys) - 6, max(ys) + 12
            # compute x extents
            xs_all = []
            for r in glossary_rows:
                xs_all.extend(r[2])
            if xs_all:
                x0, x1 = min(xs_all) - 8, max(xs_all) + 60
                rects.append({"rect": (max(0, x0), max(0, y0), x1, y1), "type": "glossary", "score": len(glossary_rows)})
        return rects

    # group cand_rows into contiguous bands
    cand_rows_sorted = sorted([r[0] for r in cand_rows])
    groups = []
    cur = [cand_rows_sorted[0]]
    for y in cand_rows_sorted[1:]:
        if y - cur[-1] <= 24:
            cur.append(y)
        else:
    ...
    for g in groups:
        ...
        for yy in g:
            for it in rows.get(yy, []):
                for s in (it.get("raw_spans") or []):
                    xs_all.append(int(round(s.get("x", 0))))
        if not xs_all:
            continue
        x0, x1 = min(xs_all) - 8, max(xs_all) + 60
        rects.append({"rect": (max(0, x0), max(0, y0), x1, y1), "type": "regular", "score": len(g)})
    return rects


def _find_priority_heading_bboxes(local_struct: dict):
    """
    Return list of rects for headings that match PRIORITY_KEYWORD_REGEXES.
    Each rect covers the heading line and following ~3 lines to include table body.
    """
    rects = []
    raw_lines = local_struct.get("raw_lines") or []
    if not raw_lines:
    ...
    for i, ln in enumerate(raw_lines):
        txt = ln.get("text") or ""
        for rx in PRIORITY_KEYWORD_REGEXES:
            if rx.search(txt):
                # build bbox from this line and next 4 lines (if present)
                y0 = ln["y"] - 6
                y1 = ln["y"] + 6
                x_vals = [s.get("x", 0) for s in (ln.get("raw_spans") or [])]
                # include next 4 lines
                for j in range(1, 5):
                    if i + j < len(raw_lines):
                        lnj = raw_lines[i + j]
                        y1 = max(y1, lnj["y"] + 6)
                        x_vals.extend([s.get("x", 0) for s in (lnj.get("raw_spans") or [])])
                if not x_vals:
                    continue
                x0, x1 = min(x_vals) - 8, max(x_vals) + 60
                rects.append((max(0, x0), max(0, y0), x1, y1))
                break
    return rects


def _render_crop_png_bytes(pdf_path: Path, page_no: int, rect_tuple):
    """Render a crop to PNG bytes using CROP_RENDER_SCALE (smaller by default)."""
    try:
        import fitz
        r = fitz.Rect(rect_tuple)
        doc = fitz.open(str(pdf_path))
        page = doc.load_page(page_no - 1)
        scale = float(CROP_RENDER_SCALE or 1.2)
        ...
        logger.debug("render crop failed: %s", e)
        return None


def _table_is_duplicate(existing_tables: list, new_table: dict) -> bool:
    """Conservative duplicate check using normalized headers and first-row overlap."""
    def norm_list(lst):
        return [_norm_token(str(x or "")) for x in (lst or []) if str(x or "").strip()]
    new_headers = norm_list(new_table.get("headers") or [])
    new_first = norm_list((new_table.get("rows") or [[]])[0])
    for et in (existing_tables or []):
        eh = norm_list(et.get("headers") or [])
        ef = norm_list((et.get("rows") or [[]])[0])
        if new_headers and eh:
            inter = set(new_headers).intersection(set(eh))
            if len(inter) / max(1, len(new_headers)) >= 0.7:
                return True
        if new_first and ef:
            if len(set(new_first).intersection(ef)) / max(1, len(new_first)) >= 0.6:
                return True
    return False

# Put these next to your other local extraction helpers (e.g. near _local_extract_text_structured)
import math

MAX_PRIORITY_CROP_CALLS = int(os.getenv("MAX_PRIORITY_CROP_CALLS", "1"))  # conservative: 1 crop max
PRIORITY_HEADING_RE = re.compile(r"\b(SIP|If you had invested|If you had invested ₹|If you had invested Rs|Systematic Investment Plan)\b", re.I)

def _detect_glossary_candidate(local_struct: dict):
    """
    More permissive glossary detector:
      - Accepts runs where each line has >=2 spans (not strictly ==2).
      - Lowers threshold (>=4 lines) so shorter glossary blocks are caught.
      - Builds an x/y rectangle covering the first two columns' x positions.
    ...
    run = []
    for ln in raw_lines:
        spans = ln.get("raw_spans") or []
        # require line has at least two span entries and some left-span text
        if len(spans) >= 2 and (ln.get("text") or "").strip() and spans[0].get("text", "").strip():
            run.append(ln)
        else:
            if len(run) >= 4:  # lowered threshold
    ...
    # choose longest candidate run
    best = max(runs, key=lambda r: len(r))

    # Collect y and x positions
    ys = [int(round(l["y"])) for l in best if isinstance(l.get("y"), (int, float))]
    first_x = []
    second_x = []
    for l in best:
        spans = l.get("raw_spans") or []
        if len(spans) >= 1:
            first_x.append(int(round(spans[0].get("x", 0))))
        if len(spans) >= 2:
            second_x.append(int(round(spans[1].get("x", 0))))

    if not ys or not first_x:
        return None

    # Conservative bounds: left from first column, right past the second column (if present)
    x0 = max(0, min(first_x) - 8)
    if second_x:
        x1 = max(second_x) + 60
    else:
        # if no clear second_x, give a reasonable width
        x1 = max(first_x) + 200

    y0 = min(ys) - 6
    y1 = max(ys) + 12

    # sanity checks: ensure width and height are positive
    if x1 <= x0 or y1 <= y0:
        return None
...
def _detect_priority_table_candidate(local_struct: dict):
    """
    Detect a candidate for SIP/dense numeric tables:
      - look for a heading matching PRIORITY_HEADING_RE (preferred), and include following 4 lines
      - otherwise find contiguous band of lines with many numeric spans
    Return a rect or None.
    """
    raw_lines = local_struct.get("raw_lines") or []
    if not raw_lines:
        return None

    # 1) heading-based detection (high priority)
    for i, ln in enumerate(raw_lines):
        txt = ln.get("text") or ""
        if PRIORITY_HEADING_RE.search(txt):
            # include this line and up to next 6 lines
            group = raw_lines[i: i + 8]
            ys = [int(round(l["y"])) for l in group]
            xs = []
            for l in group:
                for s in (l.get("raw_spans") or []):
                    xs.append(int(round(s.get("x", 0))))
            if not xs:
                continue
            x0 = max(0, min(xs) - 8)
    ...
    # 2) dense numeric band detection: contiguous rows where each row has >=2 numeric-like spans
    def is_numeric_span_text(t):
        if not t: return False
        t2 = t.replace(",", "").replace("₹", "").replace("Rs", "")
        return bool(re.match(r"^[\-\d\.]+$", t2.strip()))

    rows_by_y = {}
    for ln in raw_lines:
        y = int(round(ln["y"]))
        rows_by_y.setdefault(y, []).append(ln)
    rows_sorted = sorted(rows_by_y.items(), key=lambda kv: kv[0])
    candidate_ys = []
    for y, lines in rows_sorted:
        numeric_spans = 0
        total_spans = 0
        for ln in lines:
            for s in (ln.get("raw_spans") or []):
                total_spans += 1
                if is_numeric_span_text(s.get("text", "")):
                    numeric_spans += 1
        # a row is numeric-dense if at least 2 spans are numeric and numeric spans >= 40% of spans
        if total_spans > 0 and numeric_spans >= 2 and (numeric_spans / total_spans) >= 0.4:
            candidate_ys.append(y)
        else:
            # break runs on low-density rows
            if len(candidate_ys) >= 3:
                break
            candidate_ys = []

    if len(candidate_ys) >= 3:
        # compute bbox using neighboring lines too
        y0 = min(candidate_ys) - 8
        y1 = max(candidate_ys) + 10
        xs = []
        for yy in candidate_ys:
            for ln in rows_by_y.get(yy, []):
                for s in (ln.get("raw_spans") or []):
                    xs.append(int(round(s.get("x", 0))))
        if not xs:
            return None
...
def _local_extract_text_structured(pdf_path: Path, page_no: int) -> Dict[str, Any]:
    """
    Robust local extractor -> title, subtitle, sections, raw_lines.
    Conservative heading detection (span-level bold ratio + relative size + all-caps),
    plus deterministic postprocessing to pull obvious titles/subtitles/uppercase labels out of content.
    ...
    for b in blocks:
        for ln in b.get("lines", []):
            ...
            for s in spans:
                ...
                y = float(bbox[1]) if bbox else 0.0
                bold = ("Bold" in font or "Semibold" in font or "Medium" in font) or (bool(flags & 2))
                span_entries.append({"text": txt, "x": x, "y": y, "size": size, "bold": bool(bold), "font": font})
                parts.append(txt)
            if not span_entries:
                continue
            line_text = " ".join(parts).strip()
            if _RE_PAGE.match(line_text):   # page numbers / trivial lines
                continue
            size = max(s["size"] for s in span_entries) if span_entries else 0.0
            bold = any(s["bold"] for s in span_entries)
            raw_lines.append({
                "text": line_text,
                "size": float(size),
                "bold": bool(bold),
                "x": float(span_entries[0]["x"]),
                "y": float(span_entries[0]["y"]),
                "raw_spans": span_entries,
            })

    if not raw_lines:
        return out

    # keep raw_lines for debugging (sorted)
    raw_lines.sort(key=lambda r: (round(r["y"], 2), round(r["x"], 1)))
    out["raw_lines"] = raw_lines.copy()

    # merge lines that are very close vertically (wrapped lines)
    try:
        GAP = float(globals().get("LOCAL_MIN_PARAGRAPH_Y_GAP", 3.0))
    except Exception:
        GAP = 3.0

    merged = []
    cur = {"y": raw_lines[0]["y"], "texts": [raw_lines[0]["text"]], "raw_spans": raw_lines[0]["raw_spans"],
           "x": raw_lines[0]["x"], "size": raw_lines[0]["size"], "bold": raw_lines[0]["bold"]}
    for ln in raw_lines[1:]:
        if abs(ln["y"] - cur["y"]) <= GAP:
            cur["texts"].append(ln["text"])
            cur["raw_spans"].extend(ln["raw_spans"])
            cur["size"] = max(cur["size"], ln["size"])
            cur["bold"] = cur["bold"] or ln["bold"]
        else:
            merged.append(cur)
            cur = {"y": ln["y"], "texts": [ln["text"]], "raw_spans": ln["raw_spans"], "x": ln["x"], "size": ln["size"], "bold": ln["bold"]}
    merged.append(cur)
    ...
    for m in merged:
        ...
        final_lines.append({
            ...
            "x": float(m["x"]),
            "y": float(m["y"]),
            "raw_spans": m["raw_spans"],
        })

    # conservative title/subtitle detection (top-of-page)
    first_lines = final_lines[: min(6, len(final_lines))]
    if first_lines:
        # pick largest by size among first_lines as candidate title (only if reasonably short)
        cand = sorted(first_lines, key=lambda r: (-r["size"], r["y"]))[0]
        if cand["text"] and len(cand["text"]) < 160 and len(cand["text"].split()) <= 10:
            out["title"] = cand["text"]
            # subtitle: next line that is parenthetical or smaller
            if len(first_lines) > 1:
                nxt = sorted(first_lines, key=lambda r: r["y"])[1]
                if nxt["size"] < cand["size"] or nxt["text"].startswith("("):
                    out["subtitle"] = nxt["text"]

    # prepare stats for relative-size decisions
    sizes = [max(0.1, float(l["size"])) for l in final_lines]
    median_size = sorted(sizes)[len(sizes)//2] if sizes else 0.0

    def span_bold_ratio(spans):
        if not spans:
            return 0.0
        return sum(1 for s in spans if s.get("bold")) / max(1, len(spans))
    ...
    def starts_noise(s):
        s2 = s.lstrip()
        return bool(s2 and s2[0] in ":.-–—%") or (s2 and s2[0].isdigit())

    # strict heading decision: **conservative**
    sections = []
    ...
    for idx, ln in enumerate(final_lines):
        ...
        words = text.split()
        nwords = len(words)
        size = ln.get("size") or 0.0
        spans = ln.get("raw_spans") or []
        bold_ratio = span_bold_ratio(spans)
        allcaps = is_all_caps(text)

        # default: not heading
        is_heading = False

        # RULES (conservative):
        # 1) Strong bold ratio and short line AND relatively larger than median => heading
        if bold_ratio >= 0.70 and nwords <= 9 and size >= (median_size + 0.6):
            is_heading = True
        # 2) All-caps short line and moderately larger => heading (handles GLOSSARY/DISCLAIMER)
        elif allcaps and nwords <= 8 and size >= (median_size + 0.2):
            is_heading = True
        # 3) Very top-of-page big line (candidate title) -> treat as heading
        elif idx < 4 and size >= (median_size + 1.0) and nwords <= 12:
            is_heading = True

        # Demote headings that look like numeric/punct noise or are long (wrapped)
        if is_heading:
            letters = [c for c in text if c.isalpha()]
            alpha_ratio = len(letters) / max(1, len(text))
            if starts_noise(text) or alpha_ratio < 0.25 or len(text) > 220:
                is_heading = False

        # Demote if very small vertical gap to following line (likely wrapped)
        next_y = final_lines[idx+1]["y"] if idx+1 < len(final_lines) else None
        if is_heading and next_y is not None and (next_y - ln["y"]) <= (GAP * 1.0):
            is_heading = False

        # Finally, ensure we don't make a long content line a heading
        if is_heading and nwords > 25:
            is_heading = False
        ...
        if is_heading:
            ...
            cur = {"heading": text, "content_lines": []}
        else:
            cur["content_lines"].append(text)

    if cur and (cur["heading"] or cur["content_lines"]):
        sections.append({"heading": cur["heading"], "content": "\n".join(cur["content_lines"]).strip()})

    # compress whitespace on content and remove empty sections
    cleaned = []
    for s in sections:
    ...
    # POSTPROCESS 1: If first section has no heading but content starts with an obvious title/subtitle block,
    # split it: first all-upper short line -> heading; next parenthetical or slightly smaller -> subtitle.
    if cleaned:
        first = cleaned[0]
        if not first.get("heading") and first.get("content"):
            lines = [ln.strip() for ln in first["content"].splitlines() if ln.strip()]
            if lines:
                # case A: first line is short and ALL CAPS or many uppercase words -> title
                candidate = lines[0]
                words_c = candidate.split()
                if (is_all_caps(candidate) or len(words_c) <= 10 and sum(1 for w in words_c if w[0].isupper()) >= max(1, len(words_c)//2)):
                    # pull candidate as heading
                    cleaned[0]["heading"] = candidate
                    # remove that line from content
                    rest = "\n".join(lines[1:]).strip()
                    # pull next as subtitle if parenthetical or short and begins with '('
                    subtitle = None
                    if lines[1:]:
                        nextl = lines[1]
                        if nextl.startswith("(") and len(nextl) < 200:
                            subtitle = nextl
                            rest = "\n".join(lines[2:]).strip()
                    if subtitle:
                        out["subtitle"] = subtitle
                    cleaned[0]["content"] = _RE_MULTISPACE.sub(" ", rest).strip()

    # POSTPROCESS 2: For any section where heading is None and content starts with a short ALL-CAPS line, pull it up
    final_sections = []
    for sec in cleaned:
        h = sec.get("heading")
        ...
        if not h and c:
            ...
            if lines:
                first_line = lines[0]
                if is_all_caps(first_line) and len(first_line.split()) <= 8:
                    # promote to heading
                    h = first_line
                    c = "\n".join(lines[1:]).strip()
        final_sections.append({"heading": h, "content": _RE_MULTISPACE.sub(" ", (c or "")).strip()})

    # Final normalization: collapse duplicated neighboring headings (case-insensitive)
    normalized = []
    for sec in final_sections:
        h = sec.get("heading")
        c = sec.get("content") or ""
        if normalized and h and normalized[-1].get("heading") and normalized[-1]["heading"].strip().lower() == h.strip().lower():
            # merge content into previous
            if c:
                normalized[-1]["content"] = (normalized[-1].get("content", "") + " " + c).strip()
            continue
        normalized.append({"heading": h, "content": c})
...
def _repair_llm_tables_with_local(page_obj: dict, local_struct: dict) -> dict:
    """Conservative repair: only accept repairs that improve numeric fraction meaningfully
       and do not add empty/category rows."""
    try:
        raw_lines = local_struct.get("raw_lines") or []
        span_index = []
        for rl in raw_lines:
            for sp in rl.get("raw_spans", []):
                span_index.append({"text": sp["text"], "norm": _norm_token(sp["text"]), "x": sp["x"], "y": sp["y"]})

        for tbl in (page_obj.get("tables") or []):
            headers = tbl.get("headers") or []
            if not headers:
                continue
            # find header row candidate to infer header x positions
            header_positions = []
            for rl in raw_lines:
                nline = _norm_token(rl["text"])
                hits = sum(1 for h in headers if h and _norm_token(str(h)) in nline)
                if hits >= max(1, len(headers)//3):
                    spans = rl.get("raw_spans", [])
                    hp = []
                    for h in headers:
                        nh = _norm_token(str(h))
                        cand = next((s for s in spans if _norm_token(s["text"]) == nh), None)
                        if cand:
                            hp.append(cand["x"])
                        else:
                            found = next((si for si in span_index if si["norm"] == nh and abs(si["y"] - rl["y"]) < 4.0), None)
                            hp.append(found["x"] if found else None)
                    header_positions = hp
                    break
            if not header_positions or all(h is None for h in header_positions):
                continue

            orig_rows = tbl.get("rows") or []
            repaired_rows = []
            for row in orig_rows:
                if not row:
                    repaired_rows.append(row); continue
                label = str(row[0] or "").strip()
                if not label:
                    repaired_rows.append(row); continue
                candidate_line = next((rl for rl in raw_lines if _norm_token(label) in _norm_token(rl["text"])), None)
                if not candidate_line:
                    repaired_rows.append(row); continue
                spans = list(candidate_line.get("raw_spans", []))
                for rl2 in raw_lines:
                    if abs(rl2["y"] - candidate_line["y"]) < 6.0 and rl2 is not candidate_line:
                        spans.extend(rl2.get("raw_spans", []))
                cell_spans = []
                for s in spans:
                    t = (s.get("text") or "").strip()
                    tn = t.replace(",", "")
                    if _RE_NUMERIC.match(tn) or (len(t) <= 6 and any(ch.isdigit() for ch in t)):
                        cell_spans.append((s["x"], t.strip()))
                if not cell_spans:
                    # likely a category heading — keep original row (do not convert)
                    repaired_rows.append(row); continue
                assigned = ["" for _ in headers[1:]]
                for sx, tv in cell_spans:
                    best_i = None; best_d = None
                    for i, hx in enumerate(header_positions[1:], start=0):
                        if hx is None: continue
                        d = abs(sx - hx)
                        if best_d is None or d < best_d:
                            best_d = d; best_i = i
                    if best_i is not None:
                        assigned[best_i] = (assigned[best_i] + " " + tv).strip() if assigned[best_i] else tv
                new_row = [label] + [c.strip() if c else "" for c in assigned]
                if len(new_row) < len(headers): new_row += [""] * (len(headers) - len(new_row))
                new_row = new_row[:len(headers)]
                repaired_rows.append(new_row)

            def numeric_fraction(rows):
                num=tot=0
                for r in rows:
                    for c in r[1:]:
                        if c and _RE_NUMERIC.match(str(c).replace(",", "")): num += 1
                        tot += 1
                return (num/tot) if tot>0 else 0.0

            orig_frac = numeric_fraction(orig_rows)
            new_frac = numeric_fraction(repaired_rows)
            orig_count, new_count = len(orig_rows), len(repaired_rows)
            allowed_delta = max(2, int(REPAIR_ROW_COUNT_TOLERANCE * max(1, orig_count)))
            if new_frac >= orig_frac + float(REPAIR_MIN_IMPROVEMENT) and abs(new_count - orig_count) <= allowed_delta:
                tbl["rows"] = repaired_rows
    except Exception as e:
        logger.debug("repair error: %s", e)
    return page_obj

def _post_normalize_leading_empty_header(tbl: dict) -> dict:
    """
    Conservative post-normalizer for tables with a leading empty header "".
    Two behaviors:
      A) If the first header is empty and every row's first data column is empty,
         remove that leading header and shift rows left.
      B) If the first header is empty but rows appear RIGHT-SHIFTED (many rows
         end with "" while the first-data column should be ""), then fix by
         inserting a leading "" at index 1 and dropping the trailing cell
         (i.e. transform [label, v1, v2, ..., ""] -> [label, "", v1, v2, ...]).
    The function returns the possibly modified table (mutates in place then returns).
    """
    try:
        headers = tbl.get("headers") or []
        ...
        # Only act if the first header is empty-like
        if str(headers[0] or "").strip() != "":
            return tbl

        # Standardize rows to lists
        norm_rows = []
        for r in rows:
            if isinstance(r, list):
                norm_rows.append([("" if x is None else x) for x in r])
            else:
                norm_rows.append([("" if r is None else r)])

        # Helper: check if every row's first data column (index 1) is empty
        def first_data_all_empty(rlist):
            for r in rlist:
                if len(r) > 1 and str(r[1] or "").strip() != "":
                    return False
            return True

        # Case A: remove leading header if first-data column is empty for all rows
        if first_data_all_empty(norm_rows):
            new_headers = headers[1:]
            new_rows = []
            target_len = 1 + len(new_headers)
            for r in norm_rows:
                # remove first element (placeholder column) -> shift left
                if len(r) >= 2:
                    nr = r[1:]
                else:
                    nr = []
                # ensure target length
                if len(nr) < target_len:
                    nr = nr + [""] * (target_len - len(nr))
                else:
                    nr = nr[:target_len]
                new_rows.append(nr)
        ...
        # Case B: detect right-shift pattern and fix:
        # Condition: a significant fraction of rows have last cell empty AND first-data cell non-empty.
        total = 0
        shift_indicators = 0
        for r in norm_rows:
            total += 1
            last_cell = str(r[-1] or "").strip() if len(r) >= 1 else ""
            first_data = str(r[1] or "").strip() if len(r) > 1 else ""
            if last_cell == "" and first_data != "":
                shift_indicators += 1

        # If >= 40% rows show the pattern (tunable threshold), perform conservative shift correction
        if total > 0 and (shift_indicators / total) >= 0.4:
            # Build corrected rows by inserting "" at index 1 and dropping last cell
            corrected = []
            target_len = 1 + len(headers) - 1  # after removal of leading header we expect one fewer header
            for r in norm_rows:
                # ensure at least length 1
                lab = r[0] if len(r) > 0 else ""
                middle = r[1:-1] if len(r) > 2 else (r[1:-1] if len(r) > 1 else [])
                new_row = [lab, ""] + middle
                # pad/truncate to match expected length after header drop
                if len(new_row) < (1 + len(headers) - 1):
                    new_row = new_row + [""] * ((1 + len(headers) - 1) - len(new_row))
                else:
                    new_row = new_row[: (1 + len(headers) - 1)]
                corrected.append(new_row)
            # Finally, drop the leading empty header from headers list
            tbl["headers"] = headers[1:]
            tbl["rows"] = corrected
            return tbl

        # If neither case matched, do nothing
        return tbl

    except Exception:
        # On any error, leave table unchanged (conservative)
        return tbl



def _detect_charts_from_local_text(page_obj: dict, local_struct: dict) -> dict:
    """
    If a chart is missing, try to infer a simple chart object from local sections:
    - look for a heading followed by many numeric tokens or 'FY..' tokens
    - create a minimal chart entry with x_axis values (strings) and y_axis numeric values if found
    """
    try:
        # skip if there is already a chart
        if page_obj.get("charts"):
            return page_obj

        candidates = []
        for sec in local_struct.get("sections", []):
            # if section has many newline-separated tokens and a heading, candidate
            heading = sec.get("heading") or ""
            content = sec.get("content") or ""
            if not heading or not content:
                continue
            tokens = re.split(r"[\n,;]+", content)
            # find likely numeric tokens
            nums = [t.strip() for t in tokens if _RE_NUMERIC.match(t.strip().replace(",", ""))]
            nonnums = [t.strip() for t in tokens if not _RE_NUMERIC.match(t.strip().replace(",", "")) and t.strip()]
            if len(nums) >= 3 and len(nonnums) >= 1:
                # simple chart detected
                candidates.append((heading, nonnums[:10], [float(n.replace(",", "")) for n in nums[:25] if n.replace(",", "")]))
        if candidates:
            # create simple chart from the top candidate
            h, xvals, yvals = candidates[0]
            chart = {
                "type": "chart",
                "chart_type": "inferred",
                "title": h,
                "x_axis": {"label": None, "values": xvals},
                "y_axis": {"label": None, "values": yvals},
                "data_series": [{"name": None, "values": yvals}],
                "page_number": page_obj.get("page_number"),
            }
            page_obj.setdefault("charts", []).append(chart)
    except Exception:
        pass
    return page_obj
...
def _fix_shifted_rows_for_leading_empty_header(tbl: dict) -> dict:
    """
    Detect and fix the pattern:
      headers = ["", "May-25", "Apr-25", ...]
      rows = [ [label, v1, v2, ..., ""], ... ]
    where the trailing "" belongs to the first-data column (May-25). This function
    will conservatively rotate each row into [label, "", v1, v2, ...] and drop the
    leading empty header from headers, but only when the pattern is visible in many rows.
    """
    try:
        headers = tbl.get("headers") or []
        rows = tbl.get("rows") or []
        if not headers or not rows or str(headers[0] or "").strip() != "":
            return tbl

        # normalize rows into lists and strings
        norm = []
        for r in rows:
            if isinstance(r, list):
                norm.append([("" if x is None else x) for x in r])
            else:
        ...
        # counts: how many rows have last cell empty and first-data non-empty
        last_empty_count = 0
        first_data_nonempty_count = 0
        for r in norm:
            last_cell = str(r[-1] or "").strip() if len(r) >= 1 else ""
            first_data = str(r[1] or "").strip() if len(r) > 1 else ""
            if last_cell == "":
                last_empty_count += 1
            if first_data != "":
                first_data_nonempty_count += 1

        # Heuristic: many rows end with empty AND many rows have first-data non-empty.
        # Use threshold: at least 40% rows show trailing-empty and trailing-empty_count > first_data_empty_count
        if (last_empty_count / n) < 0.40:
            return tbl

        # Also ensure the pattern likely indicates a SHIFT: trailing-empty_count > rows having first-data empty
        first_data_empty_count = n - first_data_nonempty_count
        if last_empty_count <= first_data_empty_count:
            return tbl

        # perform conservative rotation on all rows: [label, v1, v2, ..., ""] -> [label, "", v1, v2, ...]
        new_rows = []
        for r in norm:
            # ensure row has length == len(headers) by padding
            needed = len(headers) - len(r)
            if needed > 0:
                r = r + [""] * needed
            elif needed < 0:
                r = r[: len(headers)]
            # rotation: keep label, insert trailing into index 1, then r[1:-1]
            if len(r) >= 2:
                rotated = [r[0], r[-1]] + r[1:-1]
            else:
                # fallback
                rotated = [r[0]] + [""] * (len(headers) - 1)
            # make sure rotated length == len(headers)
            if len(rotated) < len(headers):
                rotated += [""] * (len(headers) - len(rotated))
            elif len(rotated) > len(headers):
                rotated = rotated[: len(headers)]
            new_rows.append(rotated)

        # Drop the leading empty header and update rows
        tbl["headers"] = headers[1:]
        tbl["rows"] = new_rows
        return tbl
    except Exception:
...
def _convert_glossary_section_to_table(page_obj: dict) -> dict:
    """
    Find a section whose heading contains 'glossary' and convert its term/definition
    pattern into a table with headers ['Term','Definition'].

    Heuristic:
      - Lines that are short and mostly uppercase (or acronyms) are considered terms.
      - Subsequent lines until the next term are the definition.
    This is local-only and conservative (only runs when heading includes 'glossary').
    """
    try:
        secs = page_obj.get("sections") or []
        new_tables = []
        used_sections = []
        for s in secs:
            heading = (s.get("heading") or "").strip() or ""
            ...
            def is_term_candidate(line: str) -> bool:
                # Candidate if short, uppercased-ish or all caps acronym or short phrase with no punctuation.
                if len(line.split()) > 8:
                    return False
                # If it is mostly uppercase letters/digits/symbols and contains letters -> candidate
                letters = sum(1 for ch in line if ch.isalpha())
                uppers = sum(1 for ch in line if ch.isupper())
                if letters > 0 and (uppers / max(1, letters)) >= 0.6:
                    return True
                # if it's an uppercase acronym (NAV, SIP, AUM, etc.)
                if re.fullmatch(r"[A-Z0-9/&\-\s\.()]{1,30}", line) and line.upper() == line:
                    return True
                # also accept single short words with no punctuation as terms
                if len(line.split()) <= 3 and len(line) < 40 and not any(ch in line for ch in ".:;"):
                    # but prefer those that are capitalized or uppercase
                    if line == line.upper() or line[0].isupper():
                        return True
                return False
            ...
            while i < len(lines):
                ln = lines[i]
                if is_term_candidate(ln):
                    term = ln
                    i += 1
                    desc_parts = []
                    while i < len(lines) and not is_term_candidate(lines[i]):
                        desc_parts.append(lines[i])
                        i += 1
                    desc = " ".join(desc_parts).strip()
                    pairs.append((term, desc))
                else:
                    # If line not a term candidate, but next line is uppercase term, maybe this is description start
                    # Try scanning ahead
                    j = i + 1
                    found_term = None
                    while j < len(lines) and j < i + 3:
                        if is_term_candidate(lines[j]):
                            found_term = lines[j]
                            break
                        j += 1
                    if found_term:
                        # treat current line as description for previous term if exists, else skip
                        i += 1
                        continue
                    # fallback: if no term candidates discovered in whole section, try pairing pairwise: odd lines term, even lines desc
                    # We'll handle fallback after loop
                    i += 1

            # Fallback pairing if none found but lines appear alternating
            if not pairs and len(lines) >= 2:
                # Try naive pairing: odd=term, even=desc
                naive = []
                k = 0
                while k + 1 < len(lines):
                    naive.append((lines[k], lines[k + 1]))
                    k += 2
                if naive:
                    pairs = naive
            ...
            if pairs:
                tbl = {
                    "type": "table",
                    "title": heading or "Glossary",
                    "headers": ["Term", "Definition"],
                    "rows": [[t, d] for (t, d) in pairs],
                    "page_number": page_obj.get("page_number"),
                }
                # avoid duplicates: check for existing identical table
                exists = False
                for et in (page_obj.get("tables") or []):
                    if et.get("title") == tbl["title"] and len(et.get("rows", [])) == len(tbl["rows"]):
                        exists = True
                        break
                if not exists:
        ...
        if new_tables:
            page_obj.setdefault("tables", []).extend(new_tables)
            # optional: clear the section content to avoid duplicate textual extraction. Comment out if you prefer to keep it.
...
def _llm_worker_from_bytes(pdf_path: Path, pdf_stem: str, pno: int, pre: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1) initial LLM pass (tables & charts only) -- keep as-is
    page_obj = extract_info_from_image(img_bytes, pno, fitz_page=None, mime_type=mime, tables_only=SEND_TABLES_ONLY)

    if not isinstance(page_obj, dict):
        page_obj = {"page_number": pno, "title": None, "subtitle": None, "narrative_text": None, "tables": [], "charts": [], "notes": None, "error": "no_page_obj", "debug": {}}

    page_obj.setdefault("tables", []); page_obj.setdefault("charts", [])

    # 2) local structured extraction (use your existing helper)
    try:
        local_struct = _local_extract_text_structured(pdf_path, pno) or {}
    except Exception as e:
        logger.warning("Local extraction failed for %s page %d: %s", pdf_path, pno, e)
        local_struct = {}

    # 3) filter local duplicates (keep existing behavior)
    try:
        filtered_local = _filter_local_struct_by_llm(page_obj, local_struct)
    except Exception as e:
        logger.warning("Filter local struct failed for %s page %d: %s", pdf_path, pno, e)
        filtered_local = local_struct or {}

    page_obj["sections"] = filtered_local.get("sections") or []
    page_obj["narrative_text"] = None
    page_obj.setdefault("debug", {})["local_raw_lines_count"] = len(filtered_local.get("raw_lines") or local_struct.get("raw_lines") or [])

    # 4) PRIORITY: attempt *one* crop-LM if high-confidence candidate exists (glossary or SIP/dense)
    crop_calls = 0
    try:
        # Try glossary first (highest precision)
        if crop_calls < MAX_PRIORITY_CROP_CALLS:
            gl_rect = _detect_glossary_candidate(filtered_local or local_struct)
            if gl_rect:
                crop_bytes = _render_crop_png_bytes(pdf_path, pno, gl_rect)
                if crop_bytes:
                    try:
                        crop_result = extract_info_from_image(crop_bytes, pno, fitz_page=None, mime_type="image/png", tables_only=True)
                        for nt in (crop_result.get("tables") or []):
                            if not _table_is_duplicate(page_obj.get("tables", []), nt):
                                page_obj.setdefault("tables", []).append(nt)
                    except Exception as e:
                        logger.debug("Glossary crop LLM failed for %s page %d: %s", pdf_path, pno, e)
                    crop_calls += 1

        # Then try SIP/dense numeric candidate only if we haven't exhausted crop calls
        if crop_calls < MAX_PRIORITY_CROP_CALLS:
            pr_rect = _detect_priority_table_candidate(filtered_local or local_struct)
            if pr_rect:
                # skip if initial LLM already returned a table with similar headers or title
                found_similar = False
                existing_norms = set()
                for t in page_obj.get("tables") or []:
                    for h in (t.get("headers") or []):
                        nh = _norm_token(str(h))
                        if nh: existing_norms.add(nh)
                    tt = _norm_token(str(t.get("title") or ""))
                    if tt: existing_norms.add(tt)

                if any(PRIORITY_HEADING_RE.search((t.get("title") or "")) for t in page_obj.get("tables") or []):
                    found_similar = True

                if not found_similar:
                    crop_bytes = _render_crop_png_bytes(pdf_path, pno, pr_rect)
                    if crop_bytes:
                        try:
                            crop_result = extract_info_from_image(crop_bytes, pno, fitz_page=None, mime_type="image/png", tables_only=True)
                            for nt in (crop_result.get("tables") or []):
                            ...
                            logger.debug("Priority crop LLM failed for %s page %d: %s", pdf_path, pno, e)
                        crop_calls += 1
    except Exception as e:
        logger.debug("Priority-crop processing failed for %s page %d: %s", pdf_path, pno, e)

    # 5) conservative repair: try to align numeric cells only if it improves numeric fraction
    try:
        page_obj = _repair_llm_tables_with_local(page_obj, filtered_local or local_struct)
    except Exception as e:
        logger.debug("Repair failed for %s page %d: %s", pdf_path, pno, e)

    # ---- POST-REPAIR: canonicalize & targeted per-row fix for leading-empty header / shifted-columns ----
    try:
        tbls = page_obj.get("tables", []) or []
        for idx, tbl in enumerate(tbls):
            if not isinstance(tbl, dict):
                continue

            # 1) run canonical normalization (existing helper)
            try:
                res = normalize_table_rows(tbl)
                if isinstance(res, dict):
                    page_obj["tables"][idx] = res
                    tbl = page_obj["tables"][idx]
            except Exception:
                pass

            # 2) If the table has a leading empty header (common placeholder), we may need to fix rows.
            headers = tbl.get("headers") or []
            rows = tbl.get("rows") or []

            if headers and str(headers[0] or "").strip() == "" and isinstance(rows, list) and rows:
                # helper: numeric-like detector (allows commas, decimals, leading +/- and %)
                import re as _re_local
                def _is_number_like(s: Any) -> bool:
                    try:
                        if s is None:
                            return False
                        s2 = str(s).strip().replace(",", "").replace(" ", "")
                        # allow percentages like 12.3% as numeric-like as well
                        if s2.endswith("%"):
                            s2 = s2[:-1]
                        return bool(_re_local.match(r"^[+-]?\d+(\.\d+)?$", s2))
                    except Exception:
                        return False

                # perform per-row correction: only for rows that end with "" and whose first-data looks numeric
                changed_any = False
                new_rows = []
                for r in rows:
                    if not isinstance(r, list):
                        new_rows.append(r)
                        continue
                    # pad/truncate to len(headers) to avoid index errors
                    if len(r) < len(headers):
                        r = r + [""] * (len(headers) - len(r))
                    elif len(r) > len(headers):
                        r = r[: len(headers)]

                    # row pattern: last cell empty AND first-data is numeric-like
                    last_cell = str(r[-1] or "").strip()
                    first_data = r[1] if len(r) > 1 else ""
                    if last_cell == "" and _is_number_like(first_data):
                        # rotate row: insert "" at index 1 and drop the trailing empty cell
                        rotated = [r[0], ""] + r[1:-1]
                        # ensure length equals len(headers) (label + columns)
                        if len(rotated) < len(headers):
                            rotated += [""] * (len(headers) - len(rotated))
                        elif len(rotated) > len(headers):
                            rotated = rotated[: len(headers)]
                ...
                # If any rows were changed, drop the leading empty header (so headers become ["May-25", ...])
                if changed_any:
                    # remove the leading placeholder header
                    new_headers = headers[1:]
                    # After dropping header, rows should be label + len(new_headers) columns.
                    # Our rotated rows still have length == len(headers), but that's OK: rows are label+columns
                    # since downstream expects rows length == len(new_headers) + 1
                    page_obj["tables"][idx]["headers"] = new_headers
                    page_obj["tables"][idx]["rows"] = new_rows
    except Exception as e:
        logger.debug("Post-normalize table fix failed for %s page %d: %s", pdf_path, pno, e)

    # 6) Convert glossary sections (local-only) into structured tables if possible (safe)
    try:
        page_obj = _convert_glossary_section_to_table(page_obj)
    except Exception as e:
        logger.debug("Glossary->table conversion failed for %s page %d: %s", pdf_path, pno, e)

    # 7) lightweight chart detection fallback (unchanged / optional)
    try:
        page_obj = _detect_charts_from_local_text(page_obj, filtered_local or local_struct)
    except Exception as e:
        logger.debug("Chart detect failed for %s page %d: %s", pdf_path, pno, e)

    # 8) persist JSON (unchanged)
    ...
    return {"page_no": pno, "page_obj": page_obj, "sent_image_path": pre.get("sent_path"), "sent_image_mime": pre.get("mime")}
...
def _image_to_temp_pdf(image_path: Path) -> Path:
    """
    Convert an image file to a single-page temporary PDF and return the temp PDF Path.
    Uses Pillow to normalize and embed the image into a PDF page sized to the image.
    The caller is responsible for deleting the returned temp file.
    """
    import fitz  # local import to follow the pattern used elsewhere

    # Read & normalize image into PNG bytes (keeps transparency safe)
    with Image.open(image_path) as im:
        # Convert to RGBA/RGB as needed to avoid mode issues
        mode = im.mode
    ...
    tmp_path = tmp_dir / f"{image_path.stem}.{image_path.suffix.lstrip('.')}.tmp.pdf"
    try:
...
def process_one_image(image_path: Path, output_dir: Path) -> Path:
    """
    Convert the image to a temporary PDF and run the same process_one_pdf() pipeline.
    The JSON is written to output_dir / <image_stem>.json just like PDFs.
    """
    tmp_pdf = None
    try:
        tmp_pdf = _image_to_temp_pdf(image_path)
        result = process_one_pdf(tmp_pdf, output_dir)
        # ensure output filename uses original image stem
        out_file = output_dir / f"{image_path.stem}.json"
        if result and result.exists() and result != out_file:
            # If process_one_pdf wrote tmp_pdf.stem.json, rename it to use the image stem.
            try:
                result.replace(out_file)
            except Exception:
                # best-effort; ignore if replace fails
                pass
        return out_file
    finally:
        # cleanup temp pdf
...
def run_all(input_dir: str, output_dir: str):
    ...
    for p in all_files:
        ...
        if sfx == ".pdf":
            ...
            try:
                ...
                rel = p
            logger.info("Skipping (unsupported extension): %s", rel)

    if not pdfs and not images:
        logger.info("Found 0 PDF(s) and 0 image(s) in %s | LLM: %s", input_dir, _llm_provider_label())
        if skipped:
            logger.info("Also skipped %d non-supported file(s).", skipped)
        return

    logger.info("Found %d PDF(s) and %d image(s) in %s | LLM: %s", len(pdfs), len(images), input_dir, _llm_provider_label())
    if skipped:
        logger.info("Also skipped %d non-supported file(s).", skipped)

    # process PDFs first (same behavior as before)
    for pdf in pdfs:
    ...
    # then process images (apply same pipeline)
    for img in images:
        out_path = output_dir / f"{img.stem}.json"
        if out_path.exists() and not OVERWRITE_OUTPUT:
            logger.info("Skipping %s (already processed → %s)", img.name, out_path)
            continue
        try:
            logger.info("Processing image as PDF: %s", img)
            process_one_image(img, output_dir)
        except Exception as e:
            logger.warning("Failed to process image %s: %s", img, e)
